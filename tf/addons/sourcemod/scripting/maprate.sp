...
bool g_bToCloseMenu[MAXPLAYERS+1];
...
{
	name = "Map Rate",
	author = "Ryan \"FLOOR_MASTER\" Mannion, Chefe & Nanochip",
...
public OnPluginStart()
{
	...
	g_cvars[CVAR_DB_CONFIG] = CreateConVar("sm_maprate_db_config", "default", "Database configuration to use for Map Rate plugin");
	g_cvars[CVAR_VERSION] = CreateConVar("sm_maprate_version", MR_VERSION, "Map Rate Version", FCVAR_NOTIFY);
	g_cvars[CVAR_AUTORATE_TIME] = CreateConVar("sm_maprate_autorate_time", "0", "If non-zero, automatically asks dead players to rate map after they have played the map for this number of seconds");
	g_cvars[CVAR_ALLOW_REVOTE] = CreateConVar("sm_maprate_allow_revote", "1", "If non-zero, allow a user to override his/her previous vote on a map");
	g_cvars[CVAR_TABLE] = CreateConVar("sm_maprate_table", "map_ratings", "The name of the database table to use");
	g_cvars[CVAR_AUTORATE_DELAY] = CreateConVar("sm_maprate_autorate_delay", "5", "After a player dies, wait this number of seconds before asking to rate if maprate_autorate_tie is non-zero");
	g_cvars[CVAR_DISMISS] = CreateConVar("sm_maprate_dismiss", "0", "If non-zero, the first voting option will be \"Dismiss\"");
	g_cvars[CVAR_RESULTS] = CreateConVar("sm_maprate_autoresults", "1", "If non-zero, the results graph will automatically be displayed when a player rates a map");
...
public OnConfigsExecuted()
{
	...
	int serial = -1;
	if (ReadMapList(g_aMapList, serial, "default", MAPLIST_FLAG_CLEARARRAY) == null)
	{
		if (serial == -1)
		{
			LogError("Failed to load map list.");
		}
	}
}
...
{
	if (!g_bCanRateMap) return Plugin_Continue;
	if (GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER) return Plugin_Continue; // prevent dead ringer from triggering
...
{
	// I could hook player_spawn event and force close the menu, but what if they're about to select it as they are spawning in?
	// This detects if they press a movement button and then force closes it.
	if (g_bToCloseMenu[client] && IsPlayerAlive(client))
	{
...
stock CreateMenuRatings(client)
{
	...
	if (g_bCanRateMap) AddMenuItem(menu, "none", g_current_map);
...
{
	new client = param1;
	if (!g_bCanRateMap) param2++; //does this work? lol
...
public T_CreateMenuRating(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (my_rating)
	{
		Format(menu_title, sizeof(menu_title), "%s\n%T", menu_title, "Your Rating", client);
		switch (my_rating)
		{
			case 1: Format(menu_title, sizeof(menu_title), "%s %T", menu_title, "1 Star", client);
			case 2: Format(menu_title, sizeof(menu_title), "%s %T", menu_title, "2 Star", client);
			case 3: Format(menu_title, sizeof(menu_title), "%s %T", menu_title, "3 Star", client);
			case 4: Format(menu_title, sizeof(menu_title), "%s %T", menu_title, "4 Star", client);
			case 5: Format(menu_title, sizeof(menu_title), "%s %T", menu_title, "5 Star", client);
		}
	}
	SetMenuTitle(menu, menu_title);
...
public Action:Command_Rate(client, args)
{
	...
	if (!g_bCanRateMap)
	{
		MC_ReplyToCommand(client, "[{green}SM{default}] This map is not on the official map rotation therefore you cannot rate it!");
		return Plugin_Handled;
	}
...
{	
	...
	* an admin. We'll specify who in the map rate panel title. */
	...
	if (rating)
	{
		Format(title, sizeof(title), "%s\n%T", title, "Previous Rating", client);
		switch (rating)
		{
			case 1: Format(title, sizeof(title), "%s %T", title, "1 Star", client);
			case 2: Format(title, sizeof(title), "%s %T", title, "2 Star", client);
			case 3: Format(title, sizeof(title), "%s %T", title, "3 Star", client);
			case 4: Format(title, sizeof(title), "%s %T", title, "4 Star", client);
			case 5: Format(title, sizeof(title), "%s %T", title, "5 Star", client);
		}
	}
