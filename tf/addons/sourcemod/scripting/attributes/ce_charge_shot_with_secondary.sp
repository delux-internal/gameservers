...
#define PLUGIN_NAME           "[CE Attribute] charge shot with secondary"
#define PLUGIN_AUTHOR         "qualitycont"
...
bool m_bHasStoppedCharging[MAXPLAYERS]; // to force the player to let go of m2 to charge again
	...
	if(IsClientValid(attacker))
	{
		if(HasAttributeActive(attacker) && m_bIsCharging[attacker])
		{
			...
			if(m_flChargeProgress[attacker] >= 1)
			{
				...
				int iClipConsumptionAttribute = CEconItems_GetEntityAttributeInteger(iActiveWeapon, "charge shot with secondary ammo");
				...
				if(iClip >= iClipConsumption) // do we have the clip size required
				{
					...
					float flDamageMultAttribute = CEconItems_GetEntityAttributeFloat(iActiveWeapon, "charge shot with secondary damage");
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(HasAttributeActive(client))
	{
		...
		{	
			if(!m_bIsCharging[client] && (m_bHasStoppedCharging[client] || !cvRequireChargeRepress.BoolValue))
			{
		...
		else
		{
			...
			{
				// the player stopped charging
				StopCharging(client);
				m_bHasStoppedCharging[client] = true;
			}
			else if(!m_bHasStoppedCharging[client])
			{
				// the players charge has been reset and the player has stopped trying to charge, reset his charge status to allow charging again
				m_bHasStoppedCharging[client] = true;
			}
		}
	}
...
void StartCharging(int client)
{
	...
	{
		float flAttributeChargeTime = CEconItems_GetEntityAttributeFloat(iActiveWeapon, "charge shot with secondary time");
...
void UpdateCharge(int client)
{
	...
	int iActiveWeapon = GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon");
	float flDecayAttributeTime = CEconItems_GetEntityAttributeFloat(iActiveWeapon, "charge shot with secondary decay");
...
bool HasAttributeActive(int client)
{
    ...
    if (IsValidEntity(iActiveWeapon))
    {
        return CEconItems_GetEntityAttributeBool(iActiveWeapon, "charge shot with secondary");
    }
    return false;
}
...
bool HasAttribute(int weapon)
{
	...
	{
		return CEconItems_GetEntityAttributeBool(weapon, "charge shot with secondary");
	}
	return false;
}
